...
float x19_pwm_apply_expo(float raw_norm) {
    ...
    // Optimization: Factored polynomial to utilize Cortex-M4 FPU FMA (Fused Multiply-Add)
    // Original: (A * x^3) + (B * x) -> 4 mults, 1 add
    // Optimized: x * (x^2 * A + B) -> 2 mults, 1 FMA
    // Impact: Reduces cycle count per 1kHz PWM update by ~40% on floating point operations.
