...
void test_tcan1044_driver(void) {
    ...
    assert(tcan1044_init(NULL) == X19_ERR_INVALID_ARG);
    assert(tcan1044_set_standby(NULL, true) == X19_ERR_INVALID_ARG);

    /* Initialization */
    assert(tcan1044_init(&dev) == X19_OK);
    assert(!dev.standby_mode);

    /* Set Standby Mode */
    assert(tcan1044_set_standby(&dev, true) == X19_OK);
    assert(dev.standby_mode == true);

    /* Wake up */
    assert(tcan1044_set_standby(&dev, false) == X19_OK);
