...
void test_full_system_simulation(void) {
    ...
    uint8_t tx_buf[64];
    size_t tx_len = 0;
    assert(x19_can_pack_thruster_cmd(&cmd, tx_buf, sizeof(tx_buf), &tx_len) == X19_OK);
    ...
    for (int cycle = 0; cycle < 5; cycle++) {
        ...
        can_send(X19_CAN_ID_THRUSTER_CMD, tx_buf, (uint8_t)tx_len);
    ...
    while (can_receive(&rx_id, rx_data, &rx_len)) {
        if (rx_id == X19_CAN_ID_NAV_TELEMETRY) {
            found_nav = true;
            x19_nav_telemetry_t nav;
            assert(x19_can_unpack_nav_telemetry(rx_data, rx_len, &nav) == X19_OK);
