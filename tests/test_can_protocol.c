...
void test_env_telemetry_pack_unpack(void) {
    x19_env_telemetry_t original = {
        .pressure_hpa = 1013.25f, .humidity_pct = 45.5f, .temperature_c = 22.3f, .leak_flags = 0x01};
...
void test_invalid_arguments(void) {
    ...
    assert(x19_can_pack_env_telemetry(NULL, buffer, sizeof(buffer), &len) == X19_ERR_INVALID_ARG);
