...
extern float rov_pwm_apply_expo(float raw_norm);
extern uint16_t rov_pwm_step_ramp(uint16_t current_us, uint16_t target_us, uint16_t max_step_us);

void test_pwm_slew_rate_limiting(void) {
    uint16_t current_pwm = 1500;
    uint16_t target_pwm = 1900;
    uint16_t max_step = ROV_PWM_MAX_SLEW_RATE_US_PER_MS;  // 2 us/ms

    // Step 1: should increase by exactly 2 us
    current_pwm = rov_pwm_step_ramp(current_pwm, target_pwm, max_step);
...
void test_pwm_clamps(void) {
    ...
    uint16_t result = rov_pwm_step_ramp(current_pwm, 2500, 1000);
    assert(result <= ROV_PWM_MAX_US);

    // Test lower clamp (1000 us)
    result = rov_pwm_step_ramp(current_pwm, 500, 1000);
    assert(result >= ROV_PWM_MIN_US);
    printf("[PASS] test_pwm_clamps\n");
}

void test_cubic_expo_mapping(void) {
    // 0.0 stick should output 0.0
    float out_neutral = rov_pwm_apply_expo(0.0f);
    assert(fabsf(out_neutral) < 0.0001f);

    // 1.0 full stick should output 1.0
    float out_full = rov_pwm_apply_expo(1.0f);
