...
void test_cubic_expo_mapping(void) {
    ...
    float out_full_rev = x19_pwm_apply_expo(-1.0f);
    assert(fabsf(out_full_rev - -1.0f) < 0.0001f);

    // -0.5 mid-stick reverse should be softened by cubic curve (> -0.5)
    float out_mid_rev = x19_pwm_apply_expo(-0.5f);
    assert(out_mid_rev > -0.5f && out_mid_rev < 0.0f);

    // Out of bounds checks
    float out_over = x19_pwm_apply_expo(1.5f);
    assert(fabsf(out_over - 1.0f) < 0.0001f);

    float out_under = x19_pwm_apply_expo(-1.5f);
    assert(fabsf(out_under - -1.0f) < 0.0001f);
