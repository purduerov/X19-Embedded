...
class ThrusterCommand:
    ...
    def unpack(cls, data: bytes) -> "ThrusterCommand":
        if len(data) < 16:
            raise ValueError("Insufficient data for ThrusterCommand (expected 16 bytes)")
...
class SolenoidCommand:
    ...
    def unpack(cls, data: bytes) -> "SolenoidCommand":
        if len(data) < 2:
            raise ValueError("Insufficient data for SolenoidCommand (expected 2 bytes)")
...
class NavTelemetry:
    ...
    def unpack(cls, data: bytes) -> "NavTelemetry":
        if len(data) < 33:
            raise ValueError("Insufficient data for NavTelemetry (expected 33 bytes)")
...
class EnvTelemetry:
    ...
    def unpack(cls, data: bytes) -> "EnvTelemetry":
        if len(data) < 13:
            raise ValueError("Insufficient data for EnvTelemetry (expected 13 bytes)")
...
class PowerTelemetry:
    ...
    def unpack(cls, data: bytes) -> "PowerTelemetry":
        if len(data) < 20:
            raise ValueError("Insufficient data for PowerTelemetry (expected 20 bytes)")
...
def unpack_sil_can_frame(chunk: bytes) -> Tuple[int, bytes]:
    ...
    if len(chunk) < SIL_PACKET_SIZE:
        raise ValueError(f"Insufficient data for SIL frame (expected {SIL_PACKET_SIZE} bytes)")
