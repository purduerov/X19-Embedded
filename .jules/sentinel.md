...
## 2026-09-09 - Prevent Out-of-Bounds Read in SIL Bridge Network Processing
**Vulnerability:** In `tests/sil_bridge_server.c`, the `pkt->len` field extracted from unauthenticated TCP socket packets (`sil_can_packet_t`) was passed directly to the `can_send()` queue function without validating if it exceeded the 64-byte limit of the `pkt->data` payload structure.
**Learning:** Embedded systems bridges are often susceptible to memory exploits when crossing domains (TCP -> CAN). An attacker on the local network could construct a malformed packet with an arbitrarily large `len` field, causing `can_send` to perform an out-of-bounds memory read (buffer over-read) from the `rx_stream_buf` stack buffer, potentially crashing the simulation server or leaking subsequent packets on the simulated CAN bus.
**Prevention:** When parsing untrusted binary network/CAN payloads in C (e.g., `tests/sil_bridge_server.c`), extracted length fields must be explicitly bounds-checked against protocol maximums (e.g., 64 bytes for CAN FD) before use in memory operations or function calls to prevent buffer over-read/overflow vulnerabilities.
