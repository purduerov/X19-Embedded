...
## 2024-05-20 - Loop Invariant Code Motion in Thruster Control
**Learning:** Computations depending only on loop invariants (like time delta and constants) inside high-frequency control loops (e.g., 1 kHz PWM slew-rate ramping over 8 thrusters) cause unnecessary repeated multiplications and casts. While GCC -O2 may hoist these, it's not guaranteed and debug builds (-O0) suffer significantly.
**Action:** Always manually hoist loop-invariant calculations outside of critical hot loops to ensure optimal performance across all compiler targets and optimization levels.
