...
async def run_hil_test(interface: str):
    ...
    try:
        # Use virtual for tests if virtual is requested, else use socketcan
        bus_interface = "virtual" if interface == "virtual" else "socketcan"
        bus = can.Bus(channel=interface, interface=bus_interface, fd=True)
